...
export default function Leaderboard() {
  ...
  const getRankColors = (rank: number) => {
    const colors = [
      // 1st - Blue gradient
      { bg: 'bg-gradient-to-r from-[#60a5fa] to-[#3b82f6]', border: 'border-[#3b82f6]', text: 'text-white' },
      // 2nd - Green gradient 
      { bg: 'bg-gradient-to-r from-[#8BC34A] to-[#6FAE3E]', border: 'border-[#8BC34A]', text: 'text-white' },
      // 3rd - Gray/white
      { bg: 'bg-gradient-to-r from-gray-400 to-gray-500', border: 'border-gray-400', text: 'text-white' },
      // 4th - Purple
    ...
    return colors[rank] || { bg: 'bg-gradient-to-r from-[rgba(20,20,40,0.6)] to-[rgba(30,30,50,0.6)]', border: 'border-white/10', text: 'text-white' };
  };

  if (loading) {
    return (
      <div className="min-h-screen px-4 md:px-6 relative overflow-hidden" style={{ background: '#F5F7FA', paddingTop: '20px' }}>
        <div className="relative z-10 w-full">
          ...
          <div className="mb-6 w-full">
            ...
            >
              <div className="flex items-center" style={{ gap: '12px' }}>
                ...
                <div>
                  <div className="w-32 h-5 bg-white/10 rounded mb-2 animate-pulse"></div>
          ...
          {/* User Info Section Skeleton - Dark Glassmorphism */}
          <div className="mb-4 w-full rounded-xl border border-white/10 backdrop-blur-sm shadow-md overflow-hidden animate-pulse" style={{ background: 'rgba(20, 20, 40, 0.5)' }}>
            <div className="flex items-center w-full justify-between px-3 py-2 border-b border-white/10" style={{ background: 'rgba(59, 130, 246, 0.2)' }}>
              <div className="flex items-center space-x-2">
                ...
                <div className="w-10 h-5 bg-white/10 rounded-full animate-pulse"></div>
              </div>
              <div className="w-16 h-8 bg-white/10 rounded-2xl animate-pulse"></div>
            </div>
            
            <div className="flex items-center p-3 space-x-3">
              <div className="relative">
                <div className="w-12 h-12 rounded-full animate-pulse" style={{ background: 'rgba(59, 130, 246, 0.2)' }}></div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="w-28 h-5 bg-white/10 rounded mb-2 animate-pulse"></div>
                  </div>
                  <div className="text-right">
                    <div className="w-20 h-5 bg-white/10 rounded mb-1 animate-pulse"></div>
          ...
          {/* Leaderboard Skeleton - Dark Glassmorphism Cards */}
          <div className="rounded-2xl p-2 shadow-lg w-full max-w-full mx-auto">
            <div className="space-y-3">
              {[...Array(10)].map((_, index) => {
                const gradientOpacity = index < 3 ? 0.4 : 0.3;
                return (
                  <div 
                    key={index} 
                    className="flex items-center px-4 py-3 rounded-xl border border-white/10 backdrop-blur-sm animate-pulse" 
                    style={{ background: `rgba(20, 20, 40, ${gradientOpacity})` }}
                  >
                    {/* Profile Picture with Rank Badge */}
                    <div className="flex items-center space-x-3 flex-1">
                      <div className="relative">
                        <div className="w-16 h-16 rounded-full animate-pulse" style={{ background: 'rgba(59, 130, 246, 0.2)' }}></div>
                        <div className="absolute -top-1 -left-1 w-6 h-6 rounded-full animate-pulse" style={{ background: 'rgba(59, 130, 246, 0.3)' }}></div>
                      </div>
                      
                      {/* Player Info */}
                      <div className="flex-1">
                        <div className="w-32 h-5 bg-white/10 rounded mb-2 animate-pulse"></div>
                        <div className="w-20 h-3 bg-white/10 rounded mb-1 animate-pulse"></div>
                    ...
                    {/* Score */}
                    <div className="text-right">
                      <div className="w-20 h-6 bg-white/10 rounded mb-1 animate-pulse"></div>
                      <div className="w-16 h-3 bg-white/10 rounded animate-pulse"></div>
                    </div>
                  </div>
                );
  ...
  return (
    <div className="min-h-screen px-4 md:px-6 relative overflow-hidden" style={{ background: '#F5F7FA', paddingTop: '20px' }}>
      ...
      {/* Top 5 Players Banner - Redesigned */}
      <motion.div 
        className="mb-7"
      ...
      >
        ...
        >
          <div className="flex items-center" style={{ gap: '16px' }}>
            {/* Degen Icon with Glow */}
            <div className="relative">
              <div className="absolute inset-0 bg-yellow-400/30 rounded-full blur-xl"></div>
              <div className="relative w-12 h-12 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center shadow-lg">
                <span className="text-white text-xl font-bold">$</span>
              </div>
            </div>
            <div>
              <div className="text-black text-xl font-bold mb-1" style={{ fontFamily: 'var(--font-fredoka-one)', textShadow: '0 2px 10px rgba(59, 130, 246, 0.5)' }}>
                Win Up To 3200 DEGEN
              </div>
              <div className="text-black/90 text-sm" style={{ fontFamily: 'var(--font-nunito)' }}>
                Top 5 players share 3200 DEGEN rewards pool!
              </div>
            </div>
          </div>
        </div>
      ...
      {userInfo && userRank && (
        <motion.div 
          className="mb-6 rounded-2xl border border-white/10 backdrop-blur-xl shadow-2xl overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10" style={{ background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)' }}>
            <div className="flex items-center space-x-3">
              <FontAwesomeIcon icon={faUser} className="text-white text-lg" />
              <span className="text-base font-semibold text-white" style={{ fontFamily: 'var(--font-fredoka-one)' }}>Your Rank</span>
              <div className="text-sm bg-white/20 text-white px-3 py-1 rounded-full font-bold" style={{ fontFamily: 'var(--font-fredoka-one)' }}>
                #{userRank}
              </div>
            </div>
            <button
              onClick={handleShareUserInfo}
              className="relative overflow-hidden group flex items-center space-x-2 text-white px-4 py-2 rounded-2xl shadow-lg transition-all duration-300 hover:scale-105"
              style={{ background: 'linear-gradient(135deg, #8BC34A, #6FAE3E)' }}
            >
              <FontAwesomeIcon icon={faShare} className="text-sm" />
              <span className="text-sm font-medium" style={{ fontFamily: 'var(--font-fredoka-one)' }}>Share</span>
              <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 transform -skew-x-12" />
            </button>
          </div>
          ...
          <div className="flex items-center p-4 space-x-4">
            ...
            <div className="relative">
              ...
              <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-blue-400/50 shadow-lg">
                <img 
                  src={userInfo.pfpUrl} 
           ...
           <div className="flex-1 min-w-0">
             <div className="flex items-center justify-between">
               <div>
                 <p className="font-bold text-xl mb-1 truncate text-white" style={{ fontFamily: 'var(--font-fredoka-one)', textShadow: '0 2px 10px rgba(59, 130, 246, 0.5)' }}>
               ...
               {/* Score & Duration */}
               <div className="text-right">
                 <p className="text-2xl font-bold text-white mb-1" style={{ fontFamily: 'var(--font-fredoka-one)', textShadow: '0 2px 10px rgba(59, 130, 246, 0.5)' }}>
                   {(userInfo.currentSeasonScore || userInfo.score || 0).toLocaleString()}
                 </p>
                 {userInfo.duration && userInfo.duration != 0 && (
                   <p className="text-xs text-white/70 flex items-center gap-1 justify-end">
                     <FontAwesomeIcon icon={faClock} /> {formatDuration(userInfo.duration)}
                   </p>
                 )}
               </div>
             </div>
      ...
      <div className="rounded-2xl p-2 md:p-4 w-full max-w-full">
        ...
        {leaderboard.length === 0 && !loading ? (
          ...
          <div className="space-y-3">
            {leaderboard.map((entry, index) => {
              const isTopTen = index < 10 && entry.nftCount && entry.nftCount > 0;
              const rankColors = isTopTen ? getRankColors(index) : getRankColors(99);
              
              return (
              <motion.div 
                key={entry.fid} 
                className={`flex items-center px-4 py-3 rounded-xl border backdrop-blur-sm shadow-lg transition-all duration-300 hover:scale-[1.02] ${isTopTen ? rankColors.bg : ''} ${rankColors.border}`}
                style={{ background: isTopTen ? '' : 'rgba(20, 20, 40, 0.6)' }}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
