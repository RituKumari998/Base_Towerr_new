...
function WalletConnectSection() {
  ...
  return (
    <div className="space-y-5">
      {/* Header Message with Enhanced Design - Base Theme */}
      <div className="relative overflow-hidden bg-white rounded-2xl p-6 border-2 border-[#0052FF] shadow-2xl">
        {/* Decorative Elements - Base Blue */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-[#0052FF]/10 to-[#0052FF]/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
        <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-[#0052FF]/10 to-[#0052FF]/5 rounded-full blur-2xl translate-y-1/2 -translate-x-1/2" />
        
        <div className="relative z-10">
          <div className="flex items-center justify-center mb-3">
            <div className="bg-[#0052FF] p-3 rounded-2xl shadow-lg animate-bounce">
              <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
          </div>
          <h2 className="text-2xl font-extrabold text-[#0052FF] mb-2 text-center">
      ...
      {!isConnected ? (
        <div className="space-y-4">
          <button
            onClick={() => open()}
            className="group relative w-full bg-[#0052FF] hover:bg-[#0041CC] text-white font-bold py-5 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 overflow-hidden"
          >
          ...
          {/* Benefits Section - Base Theme */}
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-[#0052FF]/5 rounded-xl p-3 text-center border border-[#0052FF]/20">
              <div className="text-2xl mb-1">
                <FontAwesomeIcon icon={faBolt} className="text-[#0052FF]" />
              </div>
              <p className="text-xs text-gray-700 font-medium">Early Access</p>
            </div>
            <div className="bg-[#0052FF]/5 rounded-xl p-3 text-center border border-[#0052FF]/20">
              <div className="text-2xl mb-1">
                <FontAwesomeIcon icon={faGift} className="text-[#0052FF]" />
              </div>
              <p className="text-xs text-gray-700 font-medium">Exclusive Perks</p>
            </div>
            <div className="bg-[#0052FF]/5 rounded-xl p-3 text-center border border-[#0052FF]/20">
              <div className="text-2xl mb-1">
                <FontAwesomeIcon icon={faBell} className="text-[#0052FF]" />
              </div>
              <p className="text-xs text-gray-700 font-medium">First to Know</p>
            </div>
          </div>
        </div>
      ) : submitStatus === 'success' ? (
        <div className="space-y-4 animate-in fade-in duration-500">
          <div className="relative overflow-hidden bg-white rounded-2xl p-8 border-2 border-green-500 shadow-2xl">
            ...
            <div className="relative flex flex-col items-center justify-center">
              ...
              <h3 className="text-2xl font-extrabold text-center mb-3 text-green-600 flex items-center justify-center gap-2">
                You&apos;re In! <FontAwesomeIcon icon={faCheckCircle} className="text-green-500" />
              </h3>
              <p className="text-base text-center text-gray-800 leading-relaxed mb-2">
                Welcome to the Base Block waitlist!
              </p>
          ...
          <button
            ...
            className="w-full bg-white hover:bg-gray-50 text-gray-700 font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 border-2 border-gray-300"
          >
            Disconnect Wallet
          </button>
        </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top duration-500">
          {/* Connected Wallet Card - Base Theme */}
          <div className="relative overflow-hidden bg-white rounded-2xl p-5 border-2 border-[#0052FF] shadow-xl">
            ...
            <p className="font-mono text-base break-all bg-[#F5F7FA] px-3 py-2 rounded-lg border border-gray-300 text-gray-800">
          ...
          {!isOnBaseChain && (
            <div className="bg-yellow-50 rounded-2xl p-4 border-2 border-yellow-400 animate-in fade-in duration-300">
              <div className="flex items-start space-x-3 mb-3">
                ...
                <div className="flex-1">
                  <p className="text-yellow-800 font-bold mb-1">Wrong Network</p>
                  <p className="text-yellow-700 text-sm mb-3">Please switch to Base network to continue</p>
                  <button
                    onClick={() => switchChain({ chainId: base.id })}
                    className="w-full bg-[#0052FF] hover:bg-[#0041CC] text-white font-bold py-3 px-4 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 flex items-center justify-center space-x-2"
                  >
...
export default function Home() {
  ...
  if (isLoading) {
    return (
      <SafeAreaContainer insets={context?.client.safeAreaInsets}>
        <div className="relative flex min-h-screen w-full flex-col items-center justify-center p-4 space-y-8 overflow-hidden" style={{ background: '#F5F7FA' }}>
            ...
            {[...Array(30)].map((_, i) => (
              <div
                ...
                style={{
                  ...
                  boxShadow: `0 0 ${Math.random() * 6 + 3}px ${i % 3 === 0 ? '#0052FF' : i % 3 === 1 ? '#60a5fa' : '#ffffff'}`,
                }}
              />
            ))}
          </div>
  ...
  if (!isSDKLoaded) {
    return (
          <SafeAreaContainer insets={context?.client.safeAreaInsets}>
            <div className="relative flex w-full min-h-screen flex-col items-center justify-center p-6 overflow-hidden" style={{ background: '#F5F7FA' }}>
              {/* Sparkle animation background */}
              <div className="absolute inset-0 overflow-hidden">
                {[...Array(30)].map((_, i) => (
                  <div
                  ...
                  <button 
                    onClick={() => {
                      if (typeof window !== 'undefined') {
                        window.open('https://farcaster.xyz/~/mini-apps/launch?domain=base-tower.vercel.app', '_blank')
                      }
                    }}
